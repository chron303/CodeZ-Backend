...
// AI endpoints — all protected by Firebase ID token verification.
// uid is extracted from the verified token (req.uid), never trusted from request body.
...
async function checkAndIncrement(uid) {
  ...
  var today      = todayIST();
  var counterRef = db.collection('aiUsage').doc(uid).collection('days').doc(today);

  await db.runTransaction(async function(txn) {
    var snap    = await txn.get(counterRef);
    var current = snap.exists ? (snap.data().count || 0) : 0;
    if (current >= DAILY_LIMIT_PREMIUM) {
      var e2 = new Error(
        'Daily AI limit reached (' + DAILY_LIMIT_PREMIUM + ' requests/day). Resets at midnight IST.'
      );
      e2.status = 429;
      throw e2;
    }
    ...
    var req = https.request(opts, function(res) {
      ...
      res.on('end', function() {
        ...
        try {
          ...
          if (data.error) return reject(new Error('Gemini API error: ' + data.error.message));
...
// ─── POST /api/ai/hint ────────────────────────────────────────
// verifyToken sets req.uid from the Firebase token — no uid in body needed
...
router.post('/review', verifyToken, async function(req, res) {
  var uid = req.uid;
  ...
  var prompt = [
    ...
    '{"timeComplexity":"O(n)","spaceComplexity":"O(n)","strengths":["point 1"],"improvements":["suggestion 1"],"tip":"one key takeaway"}',
  ].join('\n');
  ...
  try {
    ...
    try {
      ...
      res.json({
        timeComplexity: 'Analyzed', spaceComplexity: 'Analyzed',
...
router.post('/studyplan', verifyToken, async function(req, res) {
  var uid = req.uid;
  ...
  var topicSummary = (topics || []).slice(0, 10)
    .map(function(t) { return t.topic + ': ' + t.solved + '/' + t.total; }).join(', ');
...
// ─── GET /api/ai/usage ────────────────────────────────────────
// Returns today's usage for the authenticated user

router.get('/usage', verifyToken, async function(req, res) {
  var uid   = req.uid;
  var today = todayIST();
  try {
    var snap = await db.collection('aiUsage').doc(uid).collection('days').doc(today).get();
